...
const formatDateDialog = (dateString) => {
  const date = new Date(dateString);
  const day = date.getDate();
  const month = date.toLocaleString("default", { month: "short" }).toUpperCase(); // Format month to three-letter abbreviation in uppercase
...
const EventsList = () => {
  ...
  const upcomingEvents = sortedEvents.filter((event) => event.date >= currentDate);
    ...
    <div className="events-list">
      ...
      >
        <DialogContent dividers style={{ padding: "0", overflow: "hidden", height:"300px" }}>
          {selectedEvent && (
            <Carousel autoPlay interval={2000} infiniteLoop showThumbs={false} showStatus={false} className="custom-carousel">
              {selectedEvent.images.map((image, index) => (
                <div key={index}>
                  <img src={image} alt={selectedEvent.title} style={{ width: '100%' }} className="carousel-image" />
                </div>
              ))}
            </Carousel>
          )}
        ...
        <DialogContent dividers style={{ textAlign: "justify", backgroundColor: "#fff1c8" }}>
          <DialogTitle style={{ padding: 0, fontWeight: "bold" }}>{selectedEvent ? selectedEvent.title : ''}</DialogTitle>
          <DialogTitle style={{ padding: 0, fontWeight: "bold" }}>{selectedEvent ? formatDateDialog(selectedEvent.date) : ''}</DialogTitle>
          {selectedEvent ? selectedEvent.description : ''}
        </DialogContent>
        <DialogActions style={{ backgroundColor: "#b7f484" }}>
          <Button style={{ color: "#24262b", fontWeight:"bold" }} onClick={handleClose}>Close</Button>
        </DialogActions>
      </Dialog>
    </div>
  );
